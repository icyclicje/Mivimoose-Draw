...
const discordRedirect = () => `${config.baseUrl}/api/auth/discord/callback`;

router.get('/auth/config', (req, res) => {
  res.json({ discord: config.discordConfigured });
});
...
router.get('/library', (req, res) => {
  ...
  const lists = Object.values(store.db.library)
    ...
    .map(l => ({ ...librarySummary(l), mine: !!(user && l.ownerId === user.id) }));
  res.json({ lists });
});
...
router.post('/library', (req, res) => {
  if (rateLimited(req, 'lib-up', 5, 60 * 60 * 1000)) {
...
router.get('/rooms', (req, res) => {
  res.json({ rooms: game.listPublicRooms() });
});
