...
function wordChosen(room, word) {
  ...
  // Hints: reveal 1 letter each, at most half the word, spread across the round.
  const skipHints = room.options.hidden || room.options.combinations;
  const wordChars = word.replace(/ /g, '').replace(/\+/g, '').length;
  const effectiveHintCount = Math.min(room.options.hintCount, Math.floor(wordChars / 2));
  const hintTimes = [];
