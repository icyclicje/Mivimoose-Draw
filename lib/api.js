...
// How many rows travel to the browser. Everyone is ranked; this only caps
// the slice that is drawn, and the viewer's own row is sent separately when
// they fall outside it.
const LB_ROWS = 200;

router.get('/leaderboard', (req, res) => {
  // Every account belongs here, including people who have not scored yet —
  // a board you cannot find yourself on is a board you stop opening. Old
  // accounts predate the stats block, so treat a missing one as zeroes.
  const users = Object.values(store.db.users).filter(u => u && u.username);
  const statOf = (u, stat) => Number(u.stats && u.stats[stat]) || 0;
  const me = auth.userForToken(bearer(req));
  const out = {};
  for (const [key, spec] of Object.entries(LB_CATEGORIES)) {
    const scored = users
      .map(u => ({ u, v: statOf(u, spec.stat) }))
      .sort((a, b) => b.v - a.v || String(a.u.username || '').localeCompare(String(b.u.username || '')));
    // Equal scores share a rank — with everyone included there are a lot of
    // ties on zero, and numbering those 47, 48, 49 would be nonsense. One
    // walk down the sorted list gives every value its rank.
    const rankByValue = new Map();
    scored.forEach((r, i) => { if (!rankByValue.has(r.v)) rankByValue.set(r.v, i + 1); });
    const rankOf = (v) => {
      if (rankByValue.has(v)) return rankByValue.get(v);
      let above = 0;
      for (const r of scored) if (r.v > v) above++; else break;   // sorted, so stop early
      return above + 1;
    };
    const ranked = scored.slice(0, LB_ROWS);
    out[key] = {
      label: spec.label,
      rows: ranked.map((r) => ({
        rank: rankOf(r.v),
    ...
    // Where do I sit, if not in the slice that was sent?
    if (me) {
      const myV = statOf(me, spec.stat);
      out[key].myRank = rankOf(myV);
      out[key].myValue = myV;
    }
  }
  res.json({ categories: out, players: users.length, shown: Math.min(users.length, LB_ROWS) });
});
...
router.get('/share/:token', (req, res) => {
  ...
  const owner = store.db.users[list.ownerId];
  res.json({
    list: {
      id: list.id,
      name: list.name,
...
router.post('/drawings', requireAuth, (req, res) => {
  ...
  store.saveDrawingFile(id, buf, kind);
...
router.delete('/drawings/:id', requireAuth, (req, res) => {
  ...
  store.deleteDrawingFile(d.id, d.kind || 'png');
...
router.get('/drawings/:id/image', (req, res) => {
  ...
  const buf = store.readDrawingFile(d.id, d.kind || 'png');
...
const LIBRARY_SORTS = {
  popular: (a, b) => b.downloads - a.downloads || b.created - a.created,
