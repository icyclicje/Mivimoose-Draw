...
function getWordChoicesWithSource(room, count, exclude = new Set()) {
  ...
  for (const passes of tiers) {
    ...
    while (result.length < count && guard++ < 2000) {
      ...
      // …then a word from it — strictly the least-used ones first, chosen
      // at random among equals. The old soft weighting let a word drift two
      // or three uses ahead of its neighbours; this is a true round-robin:
      // no word gets used N+1 times while a list-mate is still on N.
      const pool = pools[listName];
      let minUses = Infinity;
      for (const w of pool) {
        const u = usedCount[w] || 0;
        if (u < minUses) minUses = u;
      }
      const level = [];
      for (let i = 0; i < pool.length; i++) {
        if ((usedCount[pool[i]] || 0) === minUses) level.push(i);
      }
      const idx = level[Math.floor(Math.random() * level.length)];
      const word = pool[idx];
      pool.splice(idx, 1);
