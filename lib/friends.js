...
// People add each other by username now; the old 6-character code still
// works for anyone who has one saved.
function findByUsername(name) {
  const wanted = String(name || '').replace(/\s+/g, ' ').trim().toLowerCase();
  if (!wanted) return null;
  return Object.values(store.db.users)
    .find(u => String(u.username || '').toLowerCase() === wanted) || null;
}
...
module.exports = { ensureFields, friendCode, findByCode, findByUsername, userById, areFriends, sendRequest, accept, decline, remove, brief };
