...
const db = {
  ...
  stats: {},     // { hourly[], daily[], accountsByDay{} } — see lib/stats.js
};
...
// ── Drawings on disk ──
// The gallery holds round snapshots (PNG) and saved game recaps (GIF).
function drawingPath(id, ext) {
  // ids are our own hex strings — sanitize anyway
  const safeExt = ext === 'gif' ? 'gif' : 'png';
  return path.join(DRAWINGS_DIR, id.replace(/[^a-f0-9]/g, '') + '.' + safeExt);
}

function saveDrawingFile(id, buffer, ext) {
  ensureDirs();
  fs.writeFileSync(drawingPath(id, ext), buffer);
}

function readDrawingFile(id, ext) {
  const p = drawingPath(id, ext);
  if (fs.existsSync(p)) return fs.readFileSync(p);
  // Records written before GIFs existed carry no extension of their own.
  const fallback = drawingPath(id, ext === 'gif' ? 'png' : 'gif');
  return fs.existsSync(fallback) ? fs.readFileSync(fallback) : null;
}

function deleteDrawingFile(id, ext) {
  try { fs.unlinkSync(drawingPath(id, ext)); } catch (_) {}
}
