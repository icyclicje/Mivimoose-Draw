...
// Watching is cheap, so the cap is generous and separate from maxPlayers.
...
function defaultOptions() {
  return {
    ...
    relayBlind: false,       // …and the second artist is never told the word
...
// Everyone taking part: spectators are in the room but out of the rotation,
// so every piece of game logic that counts players should skip them.
function connectedPlayers(room) {
  return room.players.filter(p => p.connected && !p.spectator);
}

// Including the people just watching — for "is this room abandoned" and the
// player-count badges.
function connectedAll(room) {
  return room.players.filter(p => p.connected);
}

function spectatorsOf(room) {
...
function relaySliceSeconds(room) {
  return Math.max(3, Math.min(15, Math.round(room.options.roundTime / 12)));
}
...
// In blind relay the partner draws without ever being told the word.
function partnerKnowsWord(room) {
  return !(room.options.relayMode && room.options.relayBlind);
}
...
function getRoomPublicState(room) {
  return {
    ...
    spectatorCount: spectatorsOf(room).filter(p => p.connected).length,
    host: room.hostKey,
...
function addPlayer(room, socket, identity, { spectator = false } = {}) {
  const player = {
    spectator: !!spectator,
    key: identity.key,
    socketId: socket.id,
...
function resendChoices(room, player, socket) {
  ...
  if (isCombo) {
    if (!room.combinationPart1 && drawer.key === player.key) {
      ...
      const recipient = (room.options.coopMode && partner && partnerKnowsWord(room)) ? partner : drawer;
...
function wordPickerFor(room) {
  ...
  // In a blind relay the partner must never see the word, so the lead artist
  // does all the picking however the scores stand.
  if (!partnerKnowsWord(room)) return drawer;
...
function startRound(room) {
  ...
  while (guard++ < room.players.length + 2) {
    ...
    if (candidate.connected && !candidate.spectator) break;
    room.drawerIndex++;
  }
  if (roundsExhausted(room)) { endGame(room); return; }
  ...
  if (room.options.coopMode && connectedPlayers(room).length >= 3) {
    const others = room.players
      .map((p, i) => ({ p, i }))
      .filter(({ p, i }) => i !== room.drawerIndex && p.connected && !p.spectator);
    if (others.length > 0) {
  ...
  const wantWords = Math.max(1, room.options.wordChoices);
  ...
  // A blind partner sees none of this — not even read-only.
  const showPartner = partner && partnerKnowsWord(room);
  if (room.options.combinations) {
    ...
    if (showPartner) emitTo(partner, { words: room.wordChoices, part: 1, readOnly: true, pickerName: drawer.name });
  } else {
...
function wordChosen(room, word) {
  ...
  if (partner?.socketId) {
    // Blind relay: the partner picks up the pen with no idea what they are
    // drawing, so they get the same blanks everyone else sees.
    if (partnerKnowsWord(room)) io.to(partner.socketId).emit('yourWord', wordPayload);
    else io.to(partner.socketId).emit('blindArtist', { maskedWord: maskWord(word, room.options.showPunctuation) });
  }
  emitRelayTurn(room);
...
function joinRoomFlow(socket, identity, room, { quiet = false, spectator = false } = {}) {
  const fail = (message) => {
  ...
  // Watching does not take a player slot — there is a separate, roomier cap.
  if (spectator) {
    if (spectatorsOf(room).length >= MAX_SPECTATORS) return fail('This room already has all the spectators it can take.');
  } else if (connectedPlayers(room).length >= room.options.maxPlayers) {
    return fail('Room is full.');
  }

  const player = addPlayer(room, socket, identity, { spectator });
...
function init(_io) {
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('joinRoom', ({ code, name, avatar, quiet, spectate } = {}) => {
      if (getContext(socket)) return;
      ...
      identity.name = sanitizeName(name, identity.name);
      if (avatar) identity.avatar = sanitizeAvatar(avatar);
      joinRoomFlow(socket, identity, room, { quiet: !!quiet, spectator: !!spectate });
    });

    // Swap between playing and watching. Only in the lobby: changing the
    // cast mid-round would upend the drawing rotation.
    socket.on('setSpectator', ({ spectate } = {}) => {
      const ctx = getContext(socket);
      if (!ctx) return;
      const { room, player } = ctx;
      const want = !!spectate;
      if (player.spectator === want) return;
      if (room.state !== 'lobby') {
        return socket.emit('error', { message: 'You can only switch seats between games.' });
      }
      if (!want && connectedPlayers(room).length >= room.options.maxPlayers) {
        return socket.emit('error', { message: 'Every player slot is taken right now.' });
      }
      if (want && room.hostKey === player.key && !room.managed) {
        return socket.emit('error', { message: 'Hand the room over to someone else before you sit out.' });
      }
      player.spectator = want;
      if (want) delete room.scores[player.key];
      else if (!(player.key in room.scores)) room.scores[player.key] = 0;
      broadcastState(room);
      io.to(room.code).emit('chat', {
        system: true,
        text: want
          ? `👁️ ${player.name} is watching from the sidelines.`
    ...
    socket.on('setGameOptions', ({ options } = {}) => {
      ...
      if (options.showWordSource !== undefined) room.options.showWordSource = !!options.showWordSource;
      {
        ...
        if (options.relayBlind !== undefined) o.relayBlind = !!options.relayBlind;
        ...
        if (!o.relayMode) o.relayBlind = false;
        ...
        if (room.state !== 'lobby') {
          ...
          if (options.roundTime !== undefined && room.timeLeft > o.roundTime) {
          ...
          if (options.rounds !== undefined && o.rounds !== wasRounds) {
            io.to(room.code).emit('chat', {
              system: true,
              text: o.rounds === 0 ? '⚙️ Rounds are unlimited now.' : `⚙️ Now playing ${o.rounds} rounds.`,
            });
          }
        }
      }
    ...
    socket.on('chooseWord', ({ word } = {}) => {
      ...
      if (room.options.combinations) {
        if (!room.combinationPart1) {
          ...
          const recipient = (room.options.coopMode && partner && partnerKnowsWord(room)) ? partner : drawer;
          const coopPart2 = !!(room.options.coopMode && partner && partnerKnowsWord(room));
          ...
          const part2Picker = (room.options.coopMode && partner && partnerKnowsWord(room)) ? partner : drawer;
