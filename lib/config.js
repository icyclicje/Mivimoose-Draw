...
const config = {
  ...
  // Who holds the moderator badge when nobody has been granted one yet.
  // Setting this to your own Discord user id is strongly recommended once
  // the game is on a public server: without it the fallback is by username,
  // and a username is something anybody can type.
  adminDiscordId: String(process.env.MIVI_ADMIN_DISCORD_ID || fileConfig.adminDiscordId || '').trim(),
};

// A trailing slash here would build ".../" + "/api/..." and never match the
// redirect registered on the Discord app, so trim it once, up front.
config.baseUrl = String(config.baseUrl || '').replace(/\/+$/, '');
config.discordRedirectUri = config.baseUrl + '/api/auth/discord/callback';
